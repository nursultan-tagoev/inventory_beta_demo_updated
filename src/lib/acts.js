...
export async function createAct({ act, items, sigGiver, sigRecipient, scanFile }) {
  ...
  const rows = items.map((it) => ({ act_id: a.id, product_id: it.product_id || null, warehouse_id: it.warehouse_id || null, name: it.name, sku: it.sku || null, inv_number: it.inv || null, unit: it.unit || 'шт', qty: Number(it.qty) || 0, price: Number(it.price) || 0, sum: (Number(it.qty) || 0) * (Number(it.price) || 0), condition: it.condition || null, dept: it.dept || null }))
