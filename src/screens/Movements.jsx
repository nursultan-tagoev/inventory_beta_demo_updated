...
export default function Movements({ data, profile, can }) {
  const { movements, products, recipients, warehouses } = data
  ...
  const ops = [{ t: 'out', l: '📤 Выдать', v: 'primary' }, { t: 'return', l: '🔄 Возврат', v: 'secondary' }, { t: 'transfer', l: '⇄ Перемещение', v: 'secondary' }, { t: 'writeoff', l: '🗑 Списать', v: 'secondary' }, { t: 'in', l: '📥 Приход', v: 'success' }]
  const whName = (id) => warehouses.find((w) => w.id === id)?.name || ''
  const sub = (m) => m.type === 'transfer'
    ? `${whName(m.warehouse_id)} → ${whName(m.warehouse_to_id)}`
    : [rName(m.recipient_id), m.purpose, whName(m.warehouse_id) && 'склад ' + whName(m.warehouse_id)].filter(Boolean).join(' · ')
  return (
    <div style={{ maxWidth: 1040, margin: '0 auto', padding: '24px', animation: 'fadeUp .3s ease' }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
        {['all', 'in', 'out', 'return', 'writeoff', 'transfer'].map((t) => (
        ...
        {list.map((m, i) => (
          <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '12px 18px', borderBottom: i < list.length - 1 ? '1px solid var(--brd)' : 'none' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {sub(m) && <div style={{ fontSize: 11.5, color: 'var(--tx3)' }}>{sub(m)}</div>}
            </div>
            <span className="mono" style={{ fontWeight: 600, color: m.type === 'in' || m.type === 'return' ? 'var(--gr)' : m.type === 'transfer' ? 'var(--am-m)' : 'var(--tx)' }}>{m.type === 'in' || m.type === 'return' ? '+' : m.type === 'transfer' ? '~' : '−'}{m.qty}</span>
